...
def train_cql_on_dataset(dataset_path, model_name, n_steps=50000):
    ...
    cql.save_model(f"{model_name}.d3")
    print(f"Model saved to {model_name}.d3")

def main():
    datasets = [
        ("expert_dataset.h5", "cql_expert"),
        ("medium_dataset.h5", "cql_medium"),
        ("random_dataset.h5", "cql_random")
    ]
    
    for data_path, model_name in datasets:
        if os.path.exists(data_path):
            train_cql_on_dataset(data_path, model_name, n_steps=30000)
        else:
